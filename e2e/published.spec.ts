...
test("published September stats work without an API", async ({ page, isMobile }) => {
  ...
  await expect(page.getByLabel("Players filter").locator("option")).toHaveText(["All players","Baljeet","Chris","Mandeep","Saad","Sulav","Wilson"]);
  await page.getByLabel("Players filter").selectOption("saad");
  if (isMobile) {
    await expect(page.locator(".mobile-player").filter({hasText:"Saad"})).toContainText("15");
  } else {
    await expect(page.getByRole("row").filter({has:page.getByRole("button",{name:/Saad/})})).toContainText("15");
  }
