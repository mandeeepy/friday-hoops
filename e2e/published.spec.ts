...
test("published September stats work without an API", async ({ page, isMobile }) => {
  ...
  if (isMobile) {
    await expect(page.locator(".mobile-player").filter({hasText:"Saad"})).toContainText("29");
  } else {
    await expect(page.getByRole("row").filter({has:page.getByRole("button",{name:/Saad/})})).toContainText("29");
  }
